...
def recherche_pokemons():
    ...
    '''
    connexion = creer_connection("pokemon.db")
    curseur = connexion.cursor()
    requeteSQL = "SELECT pokemon.nom FROM pokemon ORDER BY pokemon.nom;"  # ascendant par défaut
...
def nombre_pokemons():
    ...
    connexion = creer_connection("pokemon.db")
    curseur = connexion.cursor()
    requeteSQL = "SELECT COUNT(*) FROM pokemon"
    curseur.execute(requeteSQL)
    res = curseur.fetchone()
    nbre_pokemons = res[0]
    return nbre_pokemons
...
def recherche_attributs_pokemon(nom_pokemon):
    ...
    connexion = creer_connection("pokemon.db")
    curseur = connexion.cursor()
    requeteSQL = f"""SELECT * FROM pokemon WHERE pokemon.nom="{nom_pokemon}";"""
    curseur.execute(requeteSQL)
    res = curseur.fetchone()
    ...
    pokemon.nom = res[2]
    pokemon.pv = res[3]
    pokemon.attaque = res[4]
    pokemon.defense = res[5]
    pokemon.attaque_speciale = res[6]
    pokemon.defense_speciale = res[7]
    pokemon.vitesse = res[8]
    num_evolution_de = res[9]
    pokemon.description = res[10]

    if num_evolution_de == None:
        pokemon.evolution_de = "aucun Pokemon"
    else:
        requeteSQL = f"""SELECT pokemon.nom FROM pokemon WHERE pokemon.id="{num_evolution_de}";"""
        curseur.execute(requeteSQL)
        res2 = curseur.fetchone()
        pokemon.evolution_de = res2[0]
    return pokemon
...
def recherche_evolutions(nom_pokemon):
    ...
    connexion = creer_connection("pokemon.db")
    curseur = connexion.cursor()
    requeteSQL = f"""SELECT pokemon.id FROM pokemon WHERE pokemon.nom="{nom_pokemon}";"""
    curseur.execute(requeteSQL)
    res = curseur.fetchone()
    id_pokemon = res[0]
    requeteSQL = f"""SELECT pokemon.nom FROM pokemon WHERE pokemon.evolution_de={id_pokemon} ORDER BY pokemon.nom"""
    curseur.execute(requeteSQL)
    res2 = curseur.fetchall()

    evolutions = []
    for i in res2:
        evolutions.append(i[0])
    return evolutions
...
def recherche_types(nom_pokemon):
    ...
    connexion = creer_connection("pokemon.db")
    curseur = connexion.cursor()
    requeteSQL = f"""SELECT type.nom FROM est_de_type JOIN type ON type.id=est_de_type.id_type JOIN pokemon on pokemon.id=est_de_type.id_pokemon
...
# création listebox
maListeBox = tk.Listbox(fenetre, width=39, height=10, borderwidth=0, selectborderwidth=0, selectmode='single',
                        font=maFont, highlightthickness=0)
...
label_attaque_speciale = tk.Label(fenetre, text="Attaque spéciale:", font=maFont2, foreground="black",
                                  background="white")
label_attaque_speciale.place(x=730, y=350)
label_defense_speciale = tk.Label(fenetre, text="Défense spéciale:", font=maFont2, foreground="black",
                                  background="white")
...
label_zone_description = tk.Label(fenetre, font=maFont3, foreground="black", width=42, height=4, background="white",
                                  borderwidth=0)
...
label_bouton_gros = tk.Label(fenetre, text="Start", font=maFontBouton, background="#DC0A2D", foreground="white",
                             borderwidth=0)
# placement du label
...
label_nbr_pokemons_texte = tk.Label(fenetre, text="Nombre de pokemons dans le pokedex", font=maFont, foreground="black",
                                    background="white")
# Placement 
